class SoundManager {
  ...
  // Gentle water splash for a miss
  ...
  // Futuristic digital security keypad beep
  ...
  // Heavy mechanical safe tumbler click
  public playTumblerClick() {
    this.vibrate(15);
    if (this.isMuted) return;
    const ctx = this.getContext();
    if (!ctx) return;
  ...
  // Massive hydraulic release and heavy steel vault door unlock
  public playVaultUnlock() {
    this.vibrate([30, 40, 60, 100]);
    if (this.isMuted) return;
    const ctx = this.getContext();
    if (!ctx) return;
    ...
    osc3.type = 'triangle';
    osc3.frequency.setValueAtTime(1046.5, now + 0.4);
    osc3.frequency.exponentialRampToValueAtTime(1318.5, now + 0.8);
